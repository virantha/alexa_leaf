...
def first_setup():
    ...
    local("mkvirtualenv alexa_leaf")

    # pip install packages
    with prefix('workon alexa_leaf'):
        local("pip install pytest")

def build_windows_dist():
    if os.name == 'nt':
        # Call the pyinstaller
        local("python ../pyinstaller/pyinstaller.py alexa_leaf_windows.spec --onefile")
...
def run_tests():
    ...
    with lcd(test_dir):
        ...
        t = local("py.test --cov-config .coveragerc --cov=alexa_leaf --cov-report=term --cov-report=html", capture=False)
...
def push_docs():
    ...
    """
    githubpages = "/Users/virantha/dev/githubdocs/alexa_leaf"
    ...
    with lcd("docs"):
        print("Running sphinx in docs/ and building to ~/dev/githubpages/alexa_leaf")
        local("make clean")
        local("make html")
